...
impl RngCore for ThreadRng {
    ...
    fn next_u64(&mut self) -> u64 {
        THREAD_RNG_KEY.with(|rng| unsafe { &mut *rng.get() }.next_u64())
    }

    #[inline(always)]
    fn fill_bytes(&mut self, dest: &mut [u8]) {
        THREAD_RNG_KEY.with(|rng| unsafe { &mut *rng.get() }.fill_bytes(dest))
    }

    #[inline(always)]
    fn try_fill_bytes(&mut self, dest: &mut [u8]) -> Result<(), Error> {
        THREAD_RNG_KEY.with(|rng| unsafe { &mut *rng.get() }.try_fill_bytes(dest))
    }
}
